...
import { ORGANIZATION_ROLES } from "@/lib/services/organizations/organization-service";
...
export async function POST(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  try {
    ...
    if (typeof body.inviteeUserId !== "string" || !ORGANIZATION_ROLES.includes(body.role)) return NextResponse.json({ error: "A user and valid organization role are required." }, { status: 400 });
