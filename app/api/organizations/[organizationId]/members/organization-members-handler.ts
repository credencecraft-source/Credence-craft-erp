...
export async function POST(request: Request, context: { params: Promise<{ organizationId: string }> }) {
  try {
    ...
    const invitation = await createOrganizationInvitation({ organizationId, inviteeUserId: workspaceUserId, role, invitedByUserId: user.id });
    return NextResponse.json({ invitation, inviteUrl: `${new URL(request.url).origin}/dashboard/invitations/${invitation.token}` }, { status: 201 });
  } catch (error) {
