    ...
    <Page className="max-w-5xl">
      <Section className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="erp-eyebrow">{version.version_name} / Business Type</p><h1 className="text-2xl font-bold text-slate-900">{entry.businessType.name}</h1><p className="text-sm text-slate-600">Configure the segments and pricing access for this business type inside version {version.version_name}.</p></div>
                ...
                {entry.segments.map((assignment) => (
                  <tr key={assignment.id} className="align-middle hover:bg-slate-50/70">
                    <td className="whitespace-nowrap px-4 py-3">
                      <Link href={`${canonicalPath}/${segmentUrlSegment(assignment.segment.name)}`} className="font-semibold text-slate-900 transition-colors hover:text-emerald-700">
                        {assignment.segment.name.toUpperCase()}
                      </Link>
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-3">
                      <form action={savePriceAction} className="flex items-center gap-2">
                        ...
                        <input name="price" type="number" min="0" step="0.01" defaultValue={assignment.price?.toString() ?? ""} placeholder="Plan price" className="w-28 rounded-lg border border-slate-300 px-2 py-1.5 text-xs text-slate-800" aria-label={`Price for ${assignment.segment.name}`} />
                        <Button type="submit" size="sm">Save</Button>
                      </form>
                    </td>
                    <td className="whitespace-nowrap px-4 py-3">
                      <span className={`rounded-full px-2 py-1 text-xs font-semibold ${assignment.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-500"}`}>
                        {assignment.is_active ? "Enabled" : "Disabled"}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-4 py-3 text-right">
                      <div className="flex justify-end gap-3">
                        <Link href={`${canonicalPath}/${segmentUrlSegment(assignment.segment.name)}/modulesbased-restriction`} className="text-xs font-semibold text-emerald-700 hover:underline">Open</Link>
                        <form action={toggleSegmentAction}>
                          <input type="hidden" name="assignmentId" value={assignment.id} />
                          <input type="hidden" name="isActive" value={String(!assignment.is_active)} />
                          <Button type="submit" size="sm" variant={assignment.is_active ? "danger" : "primary"}>
