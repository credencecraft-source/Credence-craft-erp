...
export default function FgSkuDetailPage({ organizationId, skuId }: { workspaceId: string; organizationId: string; skuId: string }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/masters?organizationId=${encodeURIComponent(organizationId)}`, { cache: "no-store" }).then((response) => response.json()),
    ]).then(([stockData, masterData]) => {
      if (!stockData.record) throw new Error(stockData.error || "Unable to load SKU.");
      const masterMap = Object.fromEntries((masterData.masters ?? []).map((master: { module_key: string; values: Option[] }) => [master.module_key, master.values]));
      setRecord(stockData.record); setForm(toForm(stockData.record)); setGstOptions(masterMap.gst ?? []); setHsnOptions(masterMap.hsn ?? []);
