...
export default function CreateGrnPage() {
  ...
  return <UiPage as="div"><Section className="space-y-6"><Link href={reportPath} className="text-xs font-semibold text-emerald-700">&larr; RM GRN Report</Link><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">Purchase Order Receiving</p><h1 className="mt-2 text-2xl font-bold text-slate-900">Create GRN</h1></div>{error && <Card className="border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</Card>}{message && <Card className="border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700">{message}</Card>}<Card className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><label className="text-xs font-semibold text-slate-700">Purchase Order<select value={purchaseOrderId} onChange={(event) => setPurchaseOrderId(event.target.value)} disabled={loading} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal"><option value="">{loading ? "Loading..." : "Select approved Purchase Order"}</option>{purchaseOrders.map((order) => <option key={order.id} value={order.id}>{order.purchaseOrderNo} · {order.vendor.name}</option>)}</select></label><label className="text-xs font-semibold text-slate-700">Warehouse<input value={warehouse} onChange={(event) => setWarehouse(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal" placeholder="Enter warehouse" /></label></div>{selectedOrder && <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-xs"><thead className="border-b border-slate-200 bg-slate-50 text-slate-600"><tr><th className="p-3">Raw Material</th><th className="p-3">Category</th><th className="p-3 text-right">Ordered</th><th className="p-3 text-right">Accepted</th><th className="p-3 text-right">Rejected</th></tr></thead><tbody className="divide-y divide-slate-100">{selectedOrder.lines.map((line) => <tr key={line.id}><td className="p-3 font-semibold">{line.rawMaterial || "-"}</td><td className="p-3">{line.category || "-"}</td><td className="p-3 text-right">{Number(line.quantity || 0).toLocaleString("en-IN")}</td><td className="p-3"><input type="number" min="0" value={quantities[line.id]?.accepted || ""} onChange={(event) => setLineValue(line.id, "accepted", event.target.value)} className="w-28 rounded border border-slate-300 px-2 py-1.5 text-right" /></td><td className="p-3"><input type="number" min="0" value={quantities[line.id]?.rejected || ""} onChange={(event) => setLineValue(line.id, "rejected", event.target.value)} className="w-28 rounded border border-slate-300 px-2 py-1.5 text-right" /></td></tr>)}</tbody></table></div>}<div className="flex justify-end"><button type="button" onClick={() => void submit()} disabled={saving || !selectedOrder} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Creating..." : "Create GRN"}</button></div></Card></Section></UiPage>;
}
