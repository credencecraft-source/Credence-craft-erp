...
export default async function PurchaseInvoicePage({ params }: { params: Promise<{ workspaceId: string; organizationId: string }> }) {
  const { workspaceId, organizationId } = await params;
  const base = `/dashboard/${workspaceId}/organizations/${organizationId}/finance-management/transactions`;
  const createPath = `${base}/purchase-invoice/new`;
        ...
        <Card className="border-slate-200 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-3">
            <div>
              ...
              <h2 className="mt-2 text-xl font-bold text-slate-900">Purchase Invoice Report</h2>
            </div>
          </div>
