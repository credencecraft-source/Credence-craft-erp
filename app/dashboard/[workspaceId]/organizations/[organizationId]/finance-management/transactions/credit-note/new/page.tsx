...
export default async function NewCreditNotePage({ params }: { params: Promise<{ workspaceId: string; organizationId: string }> }) {
  const { workspaceId, organizationId } = await params;
  const base = `/dashboard/${workspaceId}/organizations/${organizationId}/finance-management/transactions/credit-note`;

  return (
    <Page as="div">
      <Section className="space-y-6">
        <Link href={base} className="text-xs font-semibold text-sky-700">&larr; Credit Note</Link>
        <Card className="space-y-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-700">Create</p>
            <h1 className="mt-2 text-3xl font-bold text-slate-900">New Credit Note</h1>
          </div>
