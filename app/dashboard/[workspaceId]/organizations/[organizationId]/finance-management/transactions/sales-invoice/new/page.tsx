...
export default async function NewSalesInvoicePage({ params }: { params: Promise<{ workspaceId: string; organizationId: string }> }) {
  ...
  const base = `/dashboard/${workspaceId}/organizations/${organizationId}/finance-management/transactions/sales-invoice`;

  return (
    <Page as="div">
      <Section className="space-y-6">
        <Link href={base} className="text-xs font-semibold text-sky-700">&larr; Sales Invoice</Link>
        <Card className="space-y-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-700">Create</p>
            <h1 className="mt-2 text-3xl font-bold text-slate-900">New Sales Invoice</h1>
          </div>
