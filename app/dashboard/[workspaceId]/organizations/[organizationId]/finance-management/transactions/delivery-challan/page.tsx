...
export default async function DeliveryChallanPage({ params }: { params: Promise<{ workspaceId: string; organizationId: string }> }) {
  const { workspaceId, organizationId } = await params;
  const base = `/dashboard/${workspaceId}/organizations/${organizationId}/finance-management/transactions`;
  const createPath = `${base}/delivery-challan/new`;
      ...
      <Section className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold text-slate-900">Delivery Challan</h1>
          </div>
        ...
        <Card className="border-slate-200 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-3">
            <div>
              ...
              <h2 className="mt-2 text-xl font-bold text-slate-900">Delivery Challan Report</h2>
            </div>
          </div>
