...
type GstOption = { id: string; label: string; fields?: Record<string, unknown> };
type UomConvertOption = { id: string; value_id?: string; label: string; parent_id?: string | null; fields?: Record<string, unknown> };
type MaterialGroup = {
  key: string;
...
export type GroupedPurchaseOrder = {
  ...
  vendor: { id: string; name: string; gstin?: string | null; registeredState?: string | null };
  lines: Array<{
    id: string;
    bomItemId: string;
    orderNo: string | null;
...
export type MasterPurchaseOrder = {
  ...
  lines: Array<{ id: string; sourceGroupedPoNo: string | null; sourceOrderNo: string | null; styleName: string | null; rawMaterial?: string | null; stockUom?: string | null; groupedQty: number | null; vendorPrice: number | null; totalSpend: number | null }>;
};
...
function registrationStateCode(state: string | null | undefined, gstin: string | null | undefined) {
  ...
  return String(state ?? "").trim().toUpperCase().replace(/[.\-]+/g, " ").replace(/\s+/g, " ");
}
...
export default function StyleWisePurchaseOrderPage({ initialStage = "allocate" }: { initialStage?: StyleWiseStage }) {
  ...
  const loadGstOptions = async () => {
    const response = await fetch(`/api/organizations/${encodeURIComponent(organizationId)}/master-data/gst?includeInactive=false`, { cache: "no-store" });
    const data = await response.json();
  ...
  useEffect(() => {
    ...
    Promise.all([loadPriceApprovals(), loadGstOptions()]).catch((loadError) => setError(loadError instanceof Error ? loadError.message : "Unable to load price approvals."));
  ...
  const materialGroups = useMemo<MaterialGroup[]>(() => {
    ...
    for (const row of bomRows) {
      ...
      const group = grouped.get(key) ?? { key, rawMaterialName, stockUom: row.stockUom, category, subCategory, groupedQty: 0, rows: [] };
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-3">
      ...
      {stage === "allocate" ? (
        ...
        ) : stage === "price" ? <PriceApprovalStage groupedPurchaseOrders={groupedPurchaseOrders} gstOptions={gstOptions} loading={priceLoading} organizationId={organizationId} onUpdated={loadPriceApprovals} onApproved={() => setStage("create")} onError={setError} /> : <MasterPurchaseOrderReport masterPurchaseOrders={masterPurchaseOrders} />}
...
function MaterialGroupCard({ group, onClick }: { group: MaterialGroup; onClick: () => void }) {
  ...
  return <button type="button" onClick={onClick} className="group w-full rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-bold uppercase tracking-[0.14em] text-emerald-700">{group.category}</p><h3 className="mt-1 truncate text-base font-bold text-slate-950">{group.rawMaterialName}</h3><p className="mt-1 truncate text-xs text-slate-500">{group.subCategory}</p></div><ChevronRight className="h-4 w-4 shrink-0 text-slate-400 group-hover:text-emerald-700" /></div><div className="mt-4 grid grid-cols-3 gap-3 border-t border-slate-100 pt-3"><div><p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Stock UOM</p><p className="mt-1 text-sm font-bold text-slate-950">{text(group.stockUom)}</p></div><div><p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Grouped Qty</p><p className="mt-1 text-xl font-bold text-slate-950">{formatNumber(group.groupedQty)}</p></div><div><p className="text-[9px] font-bold uppercase tracking-wide text-slate-500">Related Orders</p><p className="mt-1 text-xl font-bold text-slate-950">{orderCount}</p></div></div><p className="mt-3 text-[10px] text-slate-500">{group.rows.length} BOM line{group.rows.length === 1 ? "" : "s"} · Click to view details</p></button>;
}
...
function PriceApprovalStage({ groupedPurchaseOrders, gstOptions, loading, organizationId, onUpdated, onApproved, onError }: { groupedPurchaseOrders: GroupedPurchaseOrder[]; gstOptions: GstOption[]; loading: boolean; organizationId: string; onUpdated: () => Promise<void>; onApproved: () => void; onError: (message: string) => void }) {
  const params = useParams<{ workspaceId: string; organizationId: string }>();
  const router = useRouter();
...
export function DetailedPriceApprovalCard({ order, gstOptions, organizationId, onUpdated, onError }: { order: GroupedPurchaseOrder; gstOptions: GstOption[]; organizationId: string; onUpdated: () => Promise<void>; onError: (message: string) => void }) {
  ...
  const [gstMasterId, setGstMasterId] = useState(() => gstOptions.find((option) => Number(option.fields?.Gst ?? option.fields?.gst) === Number(order.gst))?.id ?? "");
  const [uomConvertOptions, setUomConvertOptions] = useState<UomConvertOption[]>([]);
  const [buyingUom, setBuyingUom] = useState(order.buyingUom ?? "");
  const [convertValue, setConvertValue] = useState(String(order.convertValue ?? ""));
  ...
  useEffect(() => {
    if (gstMasterId || !gstOptions.length || order.gst === null || order.gst === undefined) return;
    const matchingOption = gstOptions.find((option) => Number(option.fields?.Gst ?? option.fields?.gst) === Number(order.gst));
    ...
    const loadUomConversions = async () => {
      try {
        const [uomResponse, conversionResponse] = await Promise.all([
          fetch(`/api/organizations/${encodeURIComponent(organizationId)}/master-data/uom?includeInactive=false`, { cache: "no-store" }),
          fetch(`/api/organizations/${encodeURIComponent(organizationId)}/master-data/stock-uom-convert?includeInactive=false`, { cache: "no-store" }),
        ]);
        const uomOptions = await uomResponse.json();
        const conversionOptions = await conversionResponse.json();
        if (!uomResponse.ok || !conversionResponse.ok) throw new Error("Unable to load UOM conversions.");
        const stockUom = String(order.stockUom ?? order.lines[0]?.stockUom ?? "").trim().toLowerCase();
        const stockUomRecord = (Array.isArray(uomOptions) ? uomOptions : []).find((option: UomConvertOption) => String(option.label ?? "").trim().toLowerCase() === stockUom);
        const matchingConversions = (Array.isArray(conversionOptions) ? conversionOptions : []).filter((option: UomConvertOption) => option.parent_id === stockUomRecord?.id || option.parent_id === stockUomRecord?.value_id);
        setUomConvertOptions(matchingConversions);
      } catch (loadError) {
        setActionError(loadError instanceof Error ? loadError.message : "Unable to load UOM conversions.");
      }
    };
  ...
  const savePrice = async () => {
    ...
    setActionError("");
    try {
      const response = await fetch(`/api/orders/procurement/${encodeURIComponent(order.id)}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ organizationId, action: "update-header", vendorPriceInr, vendorPrice: vendorPriceInr, gst, gstMasterId, hsnCode, buyingUom, convertValue }) });
      const data = await response.json();
  ...
  const buyingQty = Number(convertValue) > 0 ? headerTotalQty / Number(convertValue) : 0;
  const headerTotal = order.lines.reduce((sum, line) => sum + Number(line.groupedQty ?? 0) * Number(vendorPriceInr || line.vendorPrice || 0), 0);
  ...
  */
  ...
  const gstRate = Number(selectedGst?.fields?.Gst ?? selectedGst?.fields?.gst ?? gst ?? 0);
  const cgstRate = Number(selectedGst?.fields?.Cgst_Rate ?? selectedGst?.fields?.cgst_rate ?? gstRate / 2);
  const sgstRate = Number(selectedGst?.fields?.Sgst_Rate ?? selectedGst?.fields?.sgst_rate ?? gstRate / 2);
  const igstRate = Number(selectedGst?.fields?.Igst_Rate ?? selectedGst?.fields?.igst_rate ?? gstRate);
  const sameState = Boolean(registrationStateCode(order.organizationState, order.organizationGstin) && registrationStateCode(order.organizationState, order.organizationGstin) === registrationStateCode(order.vendor.registeredState, order.vendor.gstin));
  ...
  return <div className="erp-surface p-4"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Field label="Vendor" value={order.vendor.name} readOnly /><Field label="Raw material" value={text(order.rawMaterial)} readOnly /><Field label="Vendor Price INR" value={vendorPriceInr} onChange={setVendorPriceInr} /><Field label="Stock UOM" value={text(order.stockUom ?? order.lines[0]?.stockUom)} disabled /><label><span className="mb-1 block text-[10px] font-bold uppercase tracking-wide text-slate-500">Buying UOM lookup</span><select value={buyingUom} onChange={(event) => { const option = uomConvertOptions.find((item) => item.label === event.target.value); setBuyingUom(event.target.value); setConvertValue(String(option?.fields?.How_Many ?? option?.fields?.how_many ?? "")); }} className="w-full rounded border border-slate-300 bg-white px-2 py-1.5 text-xs"><option value="">Select buying UOM</option>{uomConvertOptions.map((option) => <option key={option.id} value={option.label}>{option.label}</option>)}</select></label><Field label="Convert Value" value={convertValue} disabled /><Field label="Buying Qty" value={formatNumber(buyingQty)} disabled /><label className="flex items-center gap-2 rounded border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-700"><input type="checkbox" checked={showTax} onChange={(event) => setShowTax(event.target.checked)} /> Show tax</label><label><span className="mb-1 block text-[10px] font-bold uppercase tracking-wide text-slate-500">GST lookup</span><select value={gstMasterId} onChange={(event) => { const option = gstOptions.find((item) => item.id === event.target.value); setGstMasterId(event.target.value); setGst(String(option?.fields?.Gst ?? option?.fields?.gst ?? "")); }} className="w-full rounded border border-slate-300 bg-white px-2 py-1.5 text-xs"><option value="">Select GST</option>{gstOptions.map((option) => <option key={option.id} value={option.id}>{option.label} ({String(option.fields?.Gst ?? option.fields?.gst ?? "-")}%)</option>)}</select></label>{showTax && <><ReadOnlyValue label="CGST" value={`${applicableCgstRate}% (${formatNumber(cgstAmount)})`} /><ReadOnlyValue label="SGST" value={`${applicableSgstRate}% (${formatNumber(sgstAmount)})`} /><ReadOnlyValue label="IGST" value={`${applicableIgstRate}% (${formatNumber(igstAmount)})`} /><ReadOnlyValue label="Total tax" value={formatNumber(totalTax)} /></>}<Field label="HSN Code" value={hsnCode} onChange={setHsnCode} /><ReadOnlyValue label="Total Qty" value={formatNumber(headerTotalQty)} /><ReadOnlyValue label="Total" value={formatNumber(headerTotal)} /></div>{actionError && <div className="mt-3 rounded border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{actionError}</div>}<div className="mt-4 overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-[10px]"><thead className="border-b border-slate-200 bg-slate-50 text-[9px] font-bold uppercase tracking-wide text-slate-500"><tr><th className="px-2 py-2">Grouping / Order</th><th className="px-2 py-2">Style</th><th className="px-2 py-2">Raw material</th><th className="px-2 py-2 text-right">Qty</th><th className="px-2 py-2 text-right">Buying Qty</th><th className="px-2 py-2 text-right">Price</th><th className="px-2 py-2 text-right">GST</th><th className="px-2 py-2">HSN Code</th><th className="px-2 py-2 text-right">Total</th></tr></thead><tbody className="divide-y divide-slate-100">{order.lines.map((line) => <tr key={line.id}><td className="px-2 py-2 font-semibold text-slate-800">{text(line.orderNo)}</td><td className="px-2 py-2">{text(line.styleName)}</td><td className="px-2 py-2 font-semibold text-slate-900">{text(line.itemName)}</td><td className="px-2 py-2 text-right">{formatNumber(line.groupedQty)}</td><td className="px-2 py-2 text-right">{formatNumber(Number(convertValue) > 0 ? Number(line.groupedQty ?? 0) / Number(convertValue) : 0)}</td><td className="px-2 py-2 text-right font-bold text-emerald-700">{formatNumber(Number(vendorPriceInr || line.vendorPrice || 0))}</td><td className="px-2 py-2 text-right">{text(gst)}</td><td className="px-2 py-2">{text(hsnCode)}</td><td className="px-2 py-2 text-right font-bold">{formatNumber(Number(line.groupedQty ?? 0) * Number(vendorPriceInr || line.vendorPrice || 0))}</td></tr>)}</tbody></table></div><div className="mt-4 flex justify-end"><button type="button" disabled={saving} onClick={savePrice} className="rounded-md bg-emerald-700 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-800 disabled:opacity-60">{saving ? "Saving..." : "Save price"}</button></div></div>;
}

function Field({ label, value, onChange, readOnly, disabled }: { label: string; value: string; onChange?: (value: string) => void; readOnly?: boolean; disabled?: boolean }) {
  return <label><span className="mb-1 block text-[10px] font-bold uppercase tracking-wide text-slate-500">{label}</span><input readOnly={readOnly} disabled={disabled} value={value} onChange={onChange ? (event) => onChange(event.target.value) : undefined} className={`w-full rounded border border-slate-300 px-2 py-1.5 text-xs outline-none focus:border-blue-500 ${readOnly || disabled ? "bg-slate-100 text-slate-600" : "bg-white"}`} /></label>;
}
