...
export default function PurchaseOrderDetailPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-4">
    ...
    <section className="erp-surface overflow-hidden"><div className="border-b border-slate-200 bg-slate-50 px-5 py-4"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-700">Subform</p><h2 className="mt-1 text-sm font-bold text-slate-950">Master Group purchase items</h2><p className="mt-1 text-xs text-slate-500">One line is shown for each selected Master Group.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead className="border-b border-slate-200 bg-white text-[9px] font-bold uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Master Group ID</th><th className="px-4 py-3">Raw material name</th><th className="px-4 py-3">Category</th><th className="px-4 py-3 text-right">Quantity</th><th className="px-4 py-3">Buying UOM</th><th className="px-4 py-3 text-right">Price</th><th className="px-4 py-3 text-right">GST</th><th className="px-4 py-3">HSN code</th><th className="px-4 py-3 text-right">Total</th></tr></thead><tbody className="divide-y divide-slate-100">{order.lines.map((line, index) => <tr key={`${line.masterGroupId}-${index}`} className="bg-white"><td className="px-4 py-3 font-semibold text-slate-800">{line.masterGroupId ?? "-"}</td><td className="px-4 py-3 font-semibold text-slate-900">{line.rawMaterial ?? "-"}</td><td className="px-4 py-3 text-slate-700">{line.category ?? "-"} / {line.subCategory ?? "-"}</td><td className="px-4 py-3 text-right">{number(line.quantity)}</td><td className="px-4 py-3">{line.buyingUom ?? "-"}</td><td className="px-4 py-3 text-right">{number(line.price)}</td><td className="px-4 py-3 text-right">{number(line.gst)}</td><td className="px-4 py-3">{line.hsnCode ?? "-"}</td><td className="px-4 py-3 text-right font-bold">{number(line.total)}</td></tr>)}</tbody><tfoot><tr className="border-t-2 border-slate-900"><td colSpan={7} className="px-4 py-4 text-right font-bold">Grand total</td><td className="px-4 py-4 text-right text-base font-bold">{number(order.total)}</td></tr></tfoot></table></div></section>
    <section className="erp-surface overflow-hidden"><div className="border-b border-slate-200 bg-slate-50 px-5 py-4"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-700">Subform</p><h2 className="mt-1 text-sm font-bold text-slate-950">Master Group purchase items</h2><p className="mt-1 text-xs text-slate-500">One line is shown for each selected Master Group.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-xs"><thead className="border-b border-slate-200 bg-white text-[9px] font-bold uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Master Group ID</th><th className="px-4 py-3">Raw material name</th><th className="px-4 py-3">Category</th><th className="px-4 py-3 text-right">Quantity</th><th className="px-4 py-3">Buying UOM</th><th className="px-4 py-3 text-right">Price</th><th className="px-4 py-3">Tax type</th><th className="px-4 py-3 text-right">CGST</th><th className="px-4 py-3 text-right">SGST</th><th className="px-4 py-3 text-right">IGST</th><th className="px-4 py-3">HSN code</th><th className="px-4 py-3 text-right">Total</th></tr></thead><tbody className="divide-y divide-slate-100">{order.lines.map((line, index) => <tr key={`${line.masterGroupId}-${index}`} className="bg-white"><td className="px-4 py-3 font-semibold text-slate-800">{line.masterGroupId ?? "-"}</td><td className="px-4 py-3 font-semibold text-slate-900">{line.rawMaterial ?? "-"}</td><td className="px-4 py-3 text-slate-700">{line.category ?? "-"} / {line.subCategory ?? "-"}</td><td className="px-4 py-3 text-right">{number(line.quantity)}</td><td className="px-4 py-3">{line.buyingUom ?? "-"}</td><td className="px-4 py-3 text-right">{number(line.price)}</td><td className="px-4 py-3 font-semibold">{line.taxType ?? "-"}</td><td className="px-4 py-3 text-right">{number(line.cgstAmount)} ({number(line.cgstRate)}%)</td><td className="px-4 py-3 text-right">{number(line.sgstAmount)} ({number(line.sgstRate)}%)</td><td className="px-4 py-3 text-right">{number(line.igstAmount)} ({number(line.igstRate)}%)</td><td className="px-4 py-3">{line.hsnCode ?? "-"}</td><td className="px-4 py-3 text-right font-bold">{number(line.total)}</td></tr>)}</tbody><tfoot><tr className="border-t-2 border-slate-900"><td colSpan={10} className="px-4 py-4 text-right font-bold">Grand total</td><td className="px-4 py-4 text-right text-base font-bold">{number(order.total)}</td></tr></tfoot></table></div></section>
  </div>;
}
