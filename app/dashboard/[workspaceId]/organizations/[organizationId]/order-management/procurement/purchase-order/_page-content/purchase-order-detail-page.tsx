...
const date = (value: string | null | undefined) =>
  value ? new Date(value).toLocaleDateString("en-IN") : "To be confirmed";
...
function HeaderField({ label, value }: { label: string; value: string }) {
  return (
    <label className="block">
      ...
      <span className="block rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-900">
        {value}
      </span>
    </label>
  );
...
export default function PurchaseOrderDetailPage() {
  ...
  useEffect(() => {
    fetch(
      `/api/orders/purchase-orders/${encodeURIComponent(params?.purchaseOrderId ?? "")}?organizationId=${encodeURIComponent(organizationId)}`,
      { cache: "no-store" },
    )
      .then(async (response) => {
        const data = await response.json();
        if (!response.ok)
          throw new Error(data?.error || "Unable to load Purchase Order.");
  ...
  if (error)
    return (
      <div className="mx-auto max-w-6xl rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
        {error}
      </div>
    );
  if (!order)
    ...
    <div className="mx-auto max-w-6xl space-y-4">
      <section className="erp-surface space-y-4 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <HeaderField label="Created date" value={date(order.createdAt)} />
          <HeaderField label="Grand total" value={number(order.total)} />
        </div>
      </section>
      <section className="erp-surface overflow-hidden">
        <div className="border-b border-slate-200 bg-slate-50 px-5 py-4">
          ...
          <h2 className="mt-1 text-sm font-bold text-slate-950">
            Master Group purchase items
          </h2>
          <p className="mt-1 text-xs text-slate-500">
            One line is shown for each selected Master Group.
          </p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1100px] text-left text-xs">
            <thead className="border-b border-slate-200 bg-white text-[9px] font-bold uppercase tracking-wide text-slate-500">
              <tr>
                <th className="px-4 py-3">Master Group ID</th>
                <th className="px-4 py-3">Raw material name</th>
                <th className="px-4 py-3">Category</th>
                <th className="px-4 py-3 text-right">Quantity</th>
                <th className="px-4 py-3">Buying UOM</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {order.lines.map((line, index) => (
                <tr key={`${line.masterGroupId}-${index}`} className="bg-white">
                  ...
                  <td className="px-4 py-3">{line.buyingUom ?? "-"}</td>
                  <td className="px-4 py-3 text-right">{number(line.price)}</td>
                  <td className="px-4 py-3 font-semibold">
                    {line.taxType ?? "-"}
                  </td>
                  <td className="px-4 py-3 text-right">
                    {number(line.cgstAmount)} ({number(line.cgstRate)}%)
                  </td>
                  <td className="px-4 py-3 text-right">
                    {number(line.sgstAmount)} ({number(line.sgstRate)}%)
                  </td>
                  <td className="px-4 py-3 text-right">
                    {number(line.igstAmount)} ({number(line.igstRate)}%)
                  </td>
                  <td className="px-4 py-3">{line.hsnCode ?? "-"}</td>
