  ...
  return (
    <div className="space-y-5 p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <SummaryMetric label="Orders" value={summary.orderCount.toString()} />
        <SummaryMetric label="Total Order Qty" value={summary.totalOrderQty.toLocaleString()} />
    ...
    <table className="w-full text-left text-sm">
      ...
      <tbody>
        {orderNumbers.map((orderNo) => (
          <tr key={orderNo} className="border-b border-slate-100">
      ...
      <tbody>
        {rows.map((row) => (
          <tr key={row.size} className="border-b border-slate-100">
            ...
            <td className="p-2 text-right font-semibold text-slate-900">{row.totalQty.toLocaleString()}</td>
          </tr>
        ))}
    ...
    <table className="w-full text-left text-sm">
      ...
      <tbody>
        {rows.map((item, idx) => {
          ...
          return (
            <tr key={`bom-${idx}`} className="border-b border-slate-100 align-top">
              ...
              <td className="p-2 text-right font-semibold text-slate-900">{item.totalRequiredQty.toLocaleString()}</td>
            </tr>
          );
        })}
