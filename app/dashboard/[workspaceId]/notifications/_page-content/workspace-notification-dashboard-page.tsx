...
type NotificationType = "orders" | "invitations";
...
export default function WorkspaceNotificationDashboardPage() {
  ...
  async function loadNotifications() {
    setLoading(true);
    const [invitationResponse, organizationResponse, shareResponse] = await Promise.all([
  ...
  const total = shares.length + invitations.length;
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl">
        <aside className="w-64 shrink-0 border-r border-slate-200 bg-white px-4 py-6">
          ...
          <nav aria-label="Notification types" className="space-y-1">
            ...
            <button type="button" onClick={() => setActiveType("invitations")} className={`flex w-full items-center justify-between rounded-lg px-3 py-3 text-left text-sm font-semibold transition ${!isOrders ? "bg-emerald-50 text-emerald-800" : "text-slate-600 hover:bg-slate-50"}`}>
        ...
        <section className="min-w-0 flex-1 px-5 py-6 sm:px-8 lg:px-10">
          <div className="flex flex-col gap-3 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">{isOrders ? "Order requests" : "Organization invitations"}</h2>
            </div>
            <span className="text-xs text-slate-500">{isOrders ? shares.length : invitations.length} pending</span>
          </div>
          ...
          ) : invitations.length === 0 ? <EmptyState label="No invitations" detail="Organization invitations will appear here when someone adds you to their team." /> :
