    ...
    fetch("/api/auth/session")
      .then(async (response) => {
        ...
        if (payload?.user) {
          ...
          setWorkspaceBase(`/workspace/${slugify(payload.user.name || "user")}`);
        }
      })
      .catch(() => undefined);
  });
            ...
            {activeModule === "backup" ? (
              ...
              <div className="space-y-5">
                ...
                {showCreateForm && (
                  <section className="rounded-2xl border border-[#dce4dc] bg-white p-6 shadow-sm">
                    <div className="mb-4">
                      <h2 className="text-xl font-semibold text-[#17372a]">Add new user</h2>
                    </div>
                    ...
                    <div className="grid gap-5 md:grid-cols-2">
                      ...
                      <label className="block text-sm font-medium text-zinc-700">
                        <span className="mb-2 block">Mobile no</span>
                        <input
                          value={form.mobile}
                ...
                <section className="rounded-2xl border border-[#dce4dc] bg-white p-6 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    <h2 className="text-xl font-semibold text-[#17372a]">User list</h2>
                    <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">
                      {users.length} users
                    </span>
                  </div>

                  <div className="overflow-hidden rounded-xl border border-[#dfe8df]">
                    <table className="min-w-full divide-y divide-[#dfe8df] bg-white text-left text-sm text-zinc-700">
                      <thead className="bg-[#f3f7f2] text-zinc-600">
                        <tr>
