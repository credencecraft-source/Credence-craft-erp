...
export const MASTER_DEFINITIONS: MasterDefinition[] = [
  ...
  { key: "sub-category", label: "Sub Category", description: "Child category values linked to category.", labelField: "sub_category", fields: [lookup("category", "Category", "category"), text("sub_category", "Sub Category", { required: true, unique: true })] },
  ...
  { key: "raw-material-sub-category", label: "Raw Material Sub Category", description: "Raw material sub category classification, independent from product sub category.", labelField: "Raw_Material_Sub_Category", fields: [lookup("Raw_Material_Category1", "Raw Material Category", "raw-material-category", { required: true }), text("Raw_Material_Sub_Category", "Raw Material Sub Category", { required: true, unique: true })] },
...
export const ORDER_LOOKUP_FIELDS = [
  ...
  lookup("category", "Product Category", "category"),
  lookup("subCategory", "Product Sub Category", "sub-category", { dependsOn: "category" }),
