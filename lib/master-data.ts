...
import { getMasterDefinition, type MasterFieldDefinition } from "@/lib/master-definitions";
export { getMasterDefinition, MASTER_DEFINITIONS } from "@/lib/master-definitions";
...
export async function listApprovalRequestsForOrganization(organizationId: string) {
  return prisma.approvalRequest.findMany({ where: { organization_id: organizationId }, orderBy: { created_at: "desc" } });
}

export async function updateApprovalRequestStatus(organizationId: string, requestId: string, status: "approved" | "rejected", reviewer?: string | null) {
  const request = await prisma.approvalRequest.findFirst({ where: { organization_id: organizationId, OR: [{ id: requestId }, { request_id: requestId }] } });
  if (!request) return null;
  return prisma.$transaction(async (transaction) => {
    const updated = await transaction.approvalRequest.update({ where: { id: request.id }, data: { status, reviewed_by: reviewer ?? null, reviewed_at: new Date() } });
    if (request.entity_ref_id) for (const delegate of Object.values(delegates)) {
      const value = await delegate.findFirst({ where: { organization_id: organizationId, value_id: request.entity_ref_id } });
      if (value) { await delegate.update({ where: { id: value.id }, data: { is_active: status === "approved" } }); break; }
    }
    return updated;
  });
}
